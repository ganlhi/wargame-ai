...
A position is entered and shown as **a compass direction and a distance from the origin ship**, the way a player reads it across the table: `840 mm NEbN`. Directions are the **32 points of the compass rose** — the same rose ship headings and the wind use, the one the movement rules are written in, so there is a single set of points across the whole app — and distances are whole millimetres. The origin ship itself simply reads *origin*.
...
- The token flow yields access tokens of about an hour and **no refresh token**, and a popup can only be opened from a click. So the last token is kept in local storage and used while it lasts, and before it runs out the app asks Google for another **silently** — the token flow with nothing shown, which Google grants as long as the browser still has a live Google session and the app's consent on record. That renewal is attempted a few minutes before expiry, whenever the tab comes back to the front or the network returns, when Drive itself refuses a token mid-sync, and again on app load, where the token from the last session has almost always lapsed. The account the tokens were last issued for is remembered — learnt from Drive, which the app already has the scope to ask — and named in every renewal, so Google goes straight to it rather than offering a chooser.
- A silent renewal fails when the player is signed out of Google or has withdrawn consent, and also when the browser refuses to open the flow away from a click. The first two need the player, so the app shows a *sign in* control and pushes the last saved state once they have signed in again; for the third it simply tries again on the next tap, where nothing is blocked and a silent renewal still shows no window. A popup is never opened on the app's own initiative.
